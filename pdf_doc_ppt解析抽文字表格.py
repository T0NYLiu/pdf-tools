...
def to_markdown(
    ...
    *,
    ...
    def get_page_output(doc, pno, textflags):
        ...
        vg_clusters0 = []
        # print(vg_clusters0,'vg_clusters0vg_clusters0vg_clusters0vg_clusters0')
...
def my_pdf_extract(file_path):
    doc = fitz.open(file_path)
    pages = range(doc.page_count)  # default page range
    md_string = to_markdown(doc, pages=pages)
    # pathlib.Path(file_path.replace('.pdf','.md')).write_bytes(md_string.encode())
    md_string = md_string.split("||||||")
    return md_string
    ...
    for path in files:
        if(path[-4:] == ".pdf"):
            try:
                ...
                doc = my_pdf_extract(path)
                f_ou = open(r"//data/zhaoshuofeng/workplace/hongan_data/研报/txt/"+path.split("/")[-1][:-4]+".txt", "w", encoding="utf8")
                if(len("\n".join(doc)) < 500):
                    continue
                f_ou.write("\n".join(doc))
                f_ou.close()
...
# dirAll(r"/data/zhaoshuofeng/workplace/hongan_data/多模态/data/文档图片数据/0530/pdf")
...
#     doc = ultra_extract(r"/data/cbk/kaopei_llm/data/test_file/青牛智胜-员工手册-第二版.pdf")
#     print("wait")
#     import os
#     # os.chdir(r'D:\work\jupyter_notebook\other\chatgpt演示\files')
#     # for file in os.listdir():
#     #     if 'docx' in file:
#     #         doc = ultra_extract(file)
#     #         print(file,len(doc))
...
#         doc = ultra_extract(file_path)
#         f_ou = open(r"D:\workplace\\法律场景\\劳动法实务文章分类_20240313_txt\\"+file_path.split("\\")[-1], "w", encoding="utf8")
#         f_ou.write("\n".join(doc))
#         f_ou.close()
